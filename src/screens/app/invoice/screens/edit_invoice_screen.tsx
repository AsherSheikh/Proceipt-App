...
const EditInvoiceScreen = () => {
  ...
  const { params: initialInvoice } =
    useRoute<RouteProp<RootStackParameterList, 'EditInvoice'>>();
  const [styles] = useTheme(Styles);
  const { update, invoices } = useInvoice();
  const { deleteInvoiceItem } = useInvoiceItem();
  const [selectedCustomer, setSelectedCustomer] = useState<Customer | null>(
    initialInvoice.customer || null,
  );
  const [hasDiscount, setHasDiscount] = useState(
    initialInvoice.discount !== null,
  );
  const [hasTax, setHasTax] = useState(initialInvoice.tax !== null);
  ...
  const [issuedAt, setIssuedAt] = useState<Date>(
    initialInvoice.issuedAt.toDate(),
  );
  const [loading, setLoading] = useState(false);
  const [dueAt, setDueAt] = useState<Date>(initialInvoice.dueAt.toDate());
  const [discount, setDiscount] = useState<Discount>({
    value: initialInvoice.discount?.value || 0,
    type: initialInvoice.discount?.type || 'percentage',
  });
  const [tax, setTax] = useState<Tax>({
    value: initialInvoice.tax?.value || 0,
    type: initialInvoice.tax?.type || 'percentage',
  });
  ...
  const [currency, setCurrency] = useState(initialInvoice.currency);
  ...
  const saveInvoice = async () => {
    ...
    const invoice: Partial<Invoice> & { id: string } = {
    ...
    try {
      ...
      Toast.show({
        type: 'success',
        text1: 'Invoice updated successfully',
      });
    ...
    <BottomSheetModalProvider>
      ...
      >
        <DatePicker
          ...
          date={new Date()}
          minimumDate={datePickerType === 'dueAt' ? new Date() : undefined}
          onConfirm={date => {
